...
long xmodem_recv(char *buf) {
  ...
  while(1) {
    if(!receiving)
      xmodem_wait(); /* wait until receive SOH */

    c = serial_recv_byte(SERIAL_DEFAULT_DEVICE);

    if(c == XMODEM_EOT) { /* End of transfer */
      serial_send_byte(SERIAL_DEFAULT_DEVICE, XMODEM_ACK);
      break;
    } else if(c == XMODEM_CAN) { /* Cancel */
      return -1;
    } else if(c == XMODEM_SOH) { /* begin receiving */
      receiving++;
